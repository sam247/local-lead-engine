...
function validateSchema(headers: string[]) {
  ...
  const extra = headers.filter(
    (header) => !expectedSet.has(header as (typeof EXPECTED_CALL_HEADERS)[number]) && !OPTIONAL_CALL_HEADERS.includes(header as (typeof OPTIONAL_CALL_HEADERS)[number])
  );
  const requiredOrderSlice = headers.filter((header) => expectedSet.has(header as (typeof EXPECTED_CALL_HEADERS)[number]));

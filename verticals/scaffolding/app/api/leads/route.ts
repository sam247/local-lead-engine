...
const ASSIGNED_TO = "Scaffolding";
const VERTICAL = "scaffolding";
...
function formatLeadId(n: number): string {
  return `SCF-${String(n).padStart(3, "0")}`;
}

function parseLeadId(leadId: string): number | null {
  const m = /^SCF-(\d{3,})$/.exec(leadId.trim());
  if (!m) return null;
  const n = Number(m[1]);
...
function buildEmailSubject(lead: LeadRecord) {
  return `New Scaffolding Lead – ${lead.lead_id} – ${lead.town} – ${lead.service}`;
}
...
export async function POST(req: Request) {
  try {
    ...
    // Staff notification inbox (override with env later if needed).
